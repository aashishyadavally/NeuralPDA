...
def vertexToStr(vertex: Vertex,identifiers: Map[Vertex,Int]): String = {
  try {
    ...
    s"${Paths.get(fileName).getFileName.toString}: ${identifiers(vertex).toString} ${vertex.value2(NodeKeys.LINE_NUMBER).toString} ${vertex.value2(NodeKeys.CODE)}"
...
def toDot(graph: ScalaGraph): String = {
  ...
  graph.E.hasLabel("AST").l.foreach { e =>
    val inV = vertexToStr(e.inVertex,vertex_identifiers).replace("\"", "\'")
    val outV = vertexToStr(e.outVertex,vertex_identifiers).replace("\"", "\'")
